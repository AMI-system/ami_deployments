...
df = pd.read_csv(r'./files/costa_rica_ami_deployments.csv')
...
# Create a map centered around Costa Rica
map_costa_rica = folium.Map(location=location, zoom_start=14, tiles=tiles, attr=attr) #location=[9.15, -79.85]
...
for i in range(0, len(df)):
    ...
    folium.Marker(
        ...
        icon=icon, 
        popup=popup
    ).add_to(map_costa_rica)

# Load GeoJSON data after adding the markers
geojson_file = r"./geojson_files/costa rica.geojson"
costa_rica_geojson = folium.GeoJson(geojson_file)
costa_rica_geojson.add_to(map_costa_rica)

# Save the map as an HTML file
map_costa_rica.save('index_costa_rica.html')
